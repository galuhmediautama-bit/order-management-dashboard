...
const FormsPage: React.FC = () => {
    ...
    useEffect(() => {
        setPage(1);
    }, [searchTerm, selectedBrandFilter, selectedProductFilter, pageSize, forms]);
    ...
    const paginationResult = useMemo(() => {
        ...
        const effectivePageSize = pageSize === 0 ? Math.max(1, filteredForms.length) : pageSize;
        return paginateArray(filteredForms, page, effectivePageSize);
    }, [filteredForms, page, pageSize]);

    const paginatedForms = paginationResult.data;
        ...
        <div className="space-y-5">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 p-5">
                ...
                <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                    <span>Menampilkan {paginatedForms.length} dari {filteredForms.length} formulir</span>
                </div>
            </div>
                    ...
                    {filteredForms.length > 0 && (
                        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-200 dark:border-slate-700">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="px-3 py-1 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-lg font-semibold text-sm">Total: {filteredForms.length}</span>
                            </div>
